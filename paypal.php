...
{
  $source = file_get_contents('http://www.rizalfakhri.com/api.php?email=' . $email);
        ...
        if(file_exists($argv[3]) && pathinfo($argv[3], PATHINFO_EXTENSION) == "txt" )
        {
          ...
          foreach ($mailist as $key => $mail)
          {
            ...
            else
            {
              ...
              fwrite($files, "[LIVE] => " . $mail . " | COUNTRY : " . $result['details']['country'] . "| CHECK ON : PP VALID CLI MODE BY RIZAL FAKHRI \r\n");
              $live++;
            }
            //var_dump($check);
            $init ++;
